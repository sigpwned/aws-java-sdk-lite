...
public enum CodecUtils {
   ...
   * <p>
   * dmurray: "It so happens that we're currently only calling this method with src.length ==
   * dest.length, in which case it works, but we could theoretically get away with passing a smaller
   * dest if we knew ahead of time that src contained some number of spaces. In that case it looks
   * like this implementation would truncate the result."
   * <p>
   * hchar: "Yes, but the truncation is the intentional behavior of this internal routine in that
   * case."
   *
   ...
   *
   * @throws IllegalArgumentException if the given decoded position is not a possible value produced
   *         via the respective encoding
   */
