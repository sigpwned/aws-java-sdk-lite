...
public final class AwsEndpoints {
  ...
  public static String toHostname(AwsEndpoint endpoint) {
    if (endpoint.getRegion().startsWith("cn-")) {
      return format("%s.%s.amazonaws.com.cn", endpoint.getService(), endpoint.getRegion());
    } else {
      return format("%s.%s.amazonaws.com", endpoint.getService(), endpoint.getRegion());
    }
  }

  public static AwsEndpoint defaultEndpoint(String service) {
    return AwsRegions.findDefaultRegion().map(region -> AwsEndpoint.of(region, service))
        .orElseThrow();
  }
}
