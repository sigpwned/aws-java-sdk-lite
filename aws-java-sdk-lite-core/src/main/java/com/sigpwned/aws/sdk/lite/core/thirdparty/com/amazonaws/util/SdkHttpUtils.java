...
public class SdkHttpUtils {
   ...
   * for use in the query portion of a URL), then applies some postfilters to fix things up per the
   * RFC. Can optionally handle strings which are meant to encode a path (ie include '/'es which
   ...
   * HTTP URIs or any port other than 443 for HTTPS URIs).
   *
   * @param uri
   *
  ...
  // Remove to avoid dependency on SignableRequest
  //
  // public static boolean usePayloadForQueryParameters(SignableRequest<?> request) {
  // boolean requestIsPOST = ModelHttpMethods.POST.equals(request.getHttpMethod());
  ...
  // return null;
  //
  // final List<Entry<String, String>> nameValuePairs = new ArrayList<Entry<String, String>>();
  //
  // for (Entry<String, List<String>> entry : requestParams.entrySet()) {
  // String parameterName = entry.getKey();
  // for (String value : entry.getValue()) {
  // nameValuePairs.add(new SimpleImmutableEntry<>(parameterName, value));
  // }
  // }
  //
