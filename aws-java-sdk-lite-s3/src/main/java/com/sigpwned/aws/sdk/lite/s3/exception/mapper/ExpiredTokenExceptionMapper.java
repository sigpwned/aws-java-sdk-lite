...
import com.sigpwned.aws.sdk.lite.s3.exception.ExpiredTokenException;
 ...
 * </pre>
 */
public class ExpiredTokenExceptionMapper implements ModelHttpBeanClientExceptionMapper {
  // <?xml version="1.0" encoding="UTF-8"?>
  // <Error>
  // <Code>ExpiredToken</Code>
  // <Message>The provided token has expired.</Message>
  ...
  // </Error>
  @Override
  public ExpiredTokenException mapException(ModelHttpRequestHead httpRequestHead,
    ...
    if (httpResponse.getStatusCode() == ModelHttpStatusCodes.BAD_REQUEST) {
      ...
      if (error.code().equals(ErrorMessages.CODE_EXPIRED_TOKEN))
        return S3Exceptions.populate(new ExpiredTokenException(), httpRequestHead,
